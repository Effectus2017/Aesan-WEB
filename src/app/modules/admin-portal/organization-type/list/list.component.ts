...
export class OrganizationTypeListComponent implements OnInit, OnDestroy, OnGenericTableHandler, OnGenericHeaderHandlers {
  ...
  ngOnInit() {
    ...
    if (resolvedData) {
      const dataWithOrder = resolvedData.organizationTypes.data.map((item, idx) => ({ ...item, displayOrderUI: idx + 1 }));
      this.tableConfig.dataSource.data = dataWithOrder;
      this.tableConfig.length = resolvedData.organizationTypes.count;
