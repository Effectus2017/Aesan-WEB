...
export class UsersListComponent implements OnInit, OnDestroy, OnGenericTableHandler, OnGenericHeaderHandlers {
  ...
  onTableDelete(event: Event, id: string) {
    ...
    const confirmation = this._fuseConfirmationService.open({
      title: 'users.list.delete.title',
      message: 'users.list.delete.message',
      actions: {
        confirm: {
          label: 'users.list.delete.confirm',
        },
      },
    });
        ...
        this._usersService.delete(requestParameters).subscribe({
          next: (result: any) => {
            switch (result.status) {
              case 202:
                this._notificationService.showSuccess('users.list.delete.success');
                this.getAll(0, this.headerConfig.formGroup.value);
                break;
              default:
                this._notificationService.showWarning('users.list.delete.error');
                break;
            }
          },
          error: (error) => {
