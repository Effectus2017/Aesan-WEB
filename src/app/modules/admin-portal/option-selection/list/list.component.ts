...
export class OptionSelectionListComponent implements OnInit, OnDestroy, OnGenericTableHandler, OnGenericHeaderHandlers {
  ...
  headerConfig: GenericHeaderConfig = {
    title: 'option-selection.list.title',
    formGroup: this._formBuilder.group({
      name: new FormControl(''),
    }),
    searchFieldShow: true,
    searchInputPlaceholder: 'option-selection.list.search.placeholder',
    goToAddButtonShow: true,
  };

  tableConfig: GenericTableConfig = {
    dataSource: new MatTableDataSource<OptionSelection>(),
    dataSourceList: [],
  ...
  ngOnInit() {
    ...
    if (resolvedData) {
      const dataWithOrder = resolvedData.optionSelections.data.map((item, idx) => ({ ...item, displayOrderUI: idx + 1 }));
      this.tableConfig.dataSource.data = dataWithOrder;
      this.tableConfig.length = resolvedData.optionSelections.count;
  ...
  getAll(index: number, form: any) {
    ...
    this._optionSelectionService.getAllOptionSelections(requestParameters).subscribe();
  }
  ...
  onTableEdit(event: Event, id: number) {
    event.stopPropagation();
    event.preventDefault();
    this._customRouterService.navigate([`option-selection/edit/${id}`]);
  }

  onAdd() {
    this._customRouterService.navigate(['option-selection/add']);
  }
}
