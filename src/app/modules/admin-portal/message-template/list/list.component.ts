...
export class MessageTemplateListComponent implements OnInit, OnDestroy, OnGenericTableHandler, OnGenericHeaderHandlers {
  ...
  headerConfig: GenericHeaderConfig = {
    title: 'message-template.list.title',
    formGroup: this._formBuilder.group({
      templateKey: new FormControl(''),
      description: new FormControl(''),
    }),
    searchFieldShow: true,
    searchInputPlaceholder: 'message-template.list.search.placeholder',
  ...
  ngOnInit() {
    ...
    if (resolvedData) {
      this.tableConfig.dataSource.data = resolvedData.messageTemplates.data;
      this.tableConfig.length = resolvedData.messageTemplates.count;
      this.tableConfig.dataSourceList = resolvedData.messageTemplates.data;
  ...
  getAll(index: number, form: any) {
    ...
    this._messageTemplateService.getAllMessageTemplates(requestParameters)
  ...
  onTableEdit(event: Event, id: number) {
    event.stopPropagation();
    event.preventDefault();
    this._customRouterService.navigate([`message-template/edit/${id}`]);
  }

  onAdd() {
    this._customRouterService.navigate(['message-template/add']);
  }
}
