...
import { CENTER_TYPE_COLUMNS_SCHEMA } from './columns-schema';
...
@Component({
    selector: 'app-admin-center-type-list',
...
export class CenterTypeListComponent implements OnInit, OnDestroy, OnGenericTableHandler, OnGenericHeaderHandlers {
  ...
  headerConfig: GenericHeaderConfig = {
    title: 'center-type.list.title',
    formGroup: this._formBuilder.group({
      name: new FormControl(''),
    }),
    searchFieldShow: true,
    searchInputPlaceholder: 'center-type.list.search.placeholder',
    submitButtonText: 'center-type.list.buttons.save',
    goToAddButtonShow: true,
  };

  tableConfig: GenericTableConfig = {
    dataSource: new MatTableDataSource<CenterType>(),
    dataSourceList: [],
    columnsSchema: CENTER_TYPE_COLUMNS_SCHEMA,
    displayedColumns: CENTER_TYPE_COLUMNS_SCHEMA.map((col) => (Array.isArray(col.key) ? col.key[0] : col.key)),
    handler: this,
    showPaginator: true,
    pageSize: 15,
  ...
  onTableEdit(event: Event, id: number) {
    event.stopPropagation();
    event.preventDefault();
    this._customRouterService.navigate([`center-type/edit/${id}`]);
  }

  onAdd() {
    this._customRouterService.navigate(['center-type/add']);
  }
}
