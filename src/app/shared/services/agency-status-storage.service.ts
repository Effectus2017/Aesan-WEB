...
export class AgencyStatusStorageService {
  ...
  setAgencyRestrictedStatus(data: { isCompleted: boolean; isExpired: boolean }): void {
    console.log('[AgencyStatusStorageService] setAgencyRestrictedStatus - Guardando estado:', data);
    ...
    console.log('[AgencyStatusStorageService] Estado guardado en localStorage:', {
      storageKey: this.STORAGE_KEY,
      hashedLength: hashed.length,
      timestamp: dataWithTimestamp.lastUpdated
    });
  }
  ...
  getAgencyRestrictedStatus(): AgencyRestrictedStatus | null {
    ...
    console.log('[AgencyStatusStorageService] getAgencyRestrictedStatus - Obteniendo estado:', {
      hasHashed: !!hashed,
      hashedLength: hashed?.length,
      storageKey: this.STORAGE_KEY
    });
    ...
    if (!hashed) {
      console.log('[AgencyStatusStorageService] No hay datos hasheados en localStorage');
      return null;
    }
    ...
    try {
      ...
      console.log('[AgencyStatusStorageService] Estado deshasheado correctamente:', {
