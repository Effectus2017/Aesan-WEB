...
test.describe('Sponsor Registration - PSAV Program Tests', () => {
  ...
  test('should fill complete PSAV sponsor registration form', async ({ page }) => {
    ...
    // 1. Seleccionar programa PSAV (campo requerido)
    const programSelect = page.locator('[data-cy=program-select]');
    if (await programSelect.isVisible()) {
      await programSelect.click();
      // Seleccionar específicamente PSAV (asumiendo que es la primera opción o buscar por texto)
      const psaVOption = page.locator('[data-cy=program-option]:has-text("PSAV"), [data-cy=program-option]:has-text("Programa de Servicios de Alimentación y Verano")').first();
      if (await psaVOption.isVisible()) {
        await psaVOption.click();
      } else {
        // Si no encuentra PSAV específicamente, seleccionar la primera opción
    ...
    if (await emailField.isVisible()) {
      await emailField.fill('prueba-psav@playwright.com');
      await expect(emailField).toHaveValue('prueba-psav@playwright.com');
    }
    ...
    if (await adminTitleField.isVisible()) {
      await adminTitleField.fill('Director Ejecutivo PSAV');
      await expect(adminTitleField).toHaveValue('Director Ejecutivo PSAV');
    }
  ...
  test('should test PSAV specific conditional fields', async ({ page }) => {
    ...
    // Verificar campos específicos de PSAV
    const psaVSpecificFields = [
      { selector: '[data-cy=at-risk-service-select]', name: 'Servicio en riesgo', condition: 'Visible cuando programas atléticos = Sí' },
      { selector: '[data-cy=national-youth-program-input]', name: 'Programa Nacional de Juventud', condition: 'Solo visible para PSAV' }
    ];

    console.log('\nCampos específicos de PSAV:');
    for (const field of psaVSpecificFields) {
