...
test.describe('PDAM - Agency Registration Flow', () => {
    
    test('01 - Successful Agency Registration (Deep Data Driven)', async ({ page }) => {
        const inputPath = path.join(__dirname, '01-Registro-PDAM.json');
        const outputPath = path.join(__dirname, '02-Login-PDAM.json');
        
        if (!fs.existsSync(inputPath)) {
            throw new Error('01-Registro-PDAM.json not found.');
        }
        ...
        console.log(`Registration successful for PDAM. Data piped to 02-Login-PDAM.json for ${randomEmail}.`);
    });
});
