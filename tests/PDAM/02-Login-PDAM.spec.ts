...
test.describe('PDAM - Login and Password Change Flow', () => {

  test('02 - Sequential Login and Pass Change for All Scenarios', async ({ page }) => {
    const inputPath = path.join(__dirname, '02-Login-PDAM.json');
    
    if (!fs.existsSync(inputPath)) {
        console.log('02-Login-PDAM.json not found. Skipping.');
        return;
    }
    ...
    for (let i = 0; i < allScenarios.length; i++) {
        ...
        console.log(`Processing Login/PassChange for PDAM scenario: ${scenario.id} (${scenario.email})`);
        ...
        console.log(`Password changed successfully for PDAM ${scenario.email}`);
